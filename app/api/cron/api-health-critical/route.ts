/**
 * GENESIS PHASE 71: CRITICAL API HEALTH CHECK CRON
 * 
 * Automated health checks for critical services every 5 minutes.
 * Monitors: Supabase, Redis, OpenAI, Gmail
 * 
 * Schedule: Every 5 minutes via Vercel Cron (cron: 0/5 * * * *)
...
// CONSTANTS
// ============================================
const CRITICAL_SERVICE_IDS = ['supabase', 'redis', 'openai', 'gmail'] as const;
...
async function storeSnapshot(report: HealthReport): Promise<void> {
  ...
  const { error } = await supabaseAdmin.rpc('insert_api_health_snapshot', {
    ...
    p_triggered_by: 'cron-critical',
    p_triggered_by_user_id: null,
  });

  if (error) {
    console.error('[Cron:APIHealthCritical] Failed to store snapshot:', error);
...
// ============================================
/**
 * POST /api/cron/api-health-critical
 * 
 * Runs critical health checks for mission-critical services.
 * 
 * Auth:
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!cronSecret) {
      console.error('[Cron:APIHealthCritical] CRON_SECRET not configured');
    ...
    if (providedSecret !== cronSecret) {
      console.warn('[Cron:APIHealthCritical] Unauthorized cron access attempt');
      return NextResponse.json(
        { error: 'Unauthorized' },
    ...
    console.log('[Cron:APIHealthCritical] Starting critical health checks...');

    // ============================================
    // 2. RUN HEALTH CHECKS (Critical Services Only)
    // ============================================
    const registry = createDefaultRegistry();
    const runner = new HealthRunner(registry, {
      defaultTimeoutMs: 10000, // 10s timeout for critical checks
      retryCount: 2, // Retry twice for transient failures
    });

    // Filter to critical services only
    const criticalChecks = CRITICAL_SERVICE_IDS.map((id) =>
      registry.getById(id)
    ).filter((check) => check !== null);

    if (criticalChecks.length === 0) {
      console.warn('[Cron:APIHealthCritical] No critical checks found');
      return NextResponse.json(
        {
          success: false,
          error: 'No critical checks configured',
          checksRun: 0,
          errors: 0,
    ...
    if (errors.length > 0) {
      console.error(
        `[Cron:APIHealthCritical] ${errors.length} critical service(s) failed:`,
    ...
    console.log(
      `[Cron:APIHealthCritical] Completed: ${validResults.length} checks, ` +
    ...
    console.error('[Cron:APIHealthCritical] Execution failed:', error);
