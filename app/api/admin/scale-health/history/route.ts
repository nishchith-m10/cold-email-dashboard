/**
 * PHASE 44 - GET /api/admin/scale-health/history
 * 
 * Returns historical scale metrics with trend calculations.
 * Super Admin only.
 */
...
export async function GET(req: NextRequest) {
  try {
    ...
    const { snapshots, trends } = await service.getMetricsHistory(days);

    return NextResponse.json(
      { success: true, snapshots, trends },
      { headers: API_HEADERS }
    );
  } catch (error) {
    /* eslint-disable-next-line no-console */
    console.error('[ScaleHealth] History error:', error);
    return NextResponse.json(
