 ...
 * Endpoint: POST /api/admin/migration/cutover/execute
 *
 * Execute production cutover for a workspace migration.
 * Switches application to use target table (blue-green deployment).
 *
 * Auth: Super Admin only
 * Body: { workspaceId }
 * Query Params: dryRun=true (simulates without actually switching)
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Execute cutover using orchestrator
    const orchestrator = new MigrationOrchestrator(db);
    const result = await orchestrator.executeCutover(workspaceId);
    ...
    return NextResponse.json(
      {
        ...
        data: result,
        message: dryRun
          ? 'Cutover executed (DRY RUN - no production changes)'
          : 'Cutover executed successfully - now using target table',
      },
      { status: 200, headers: API_HEADERS }
    );
  } catch (error) {
    /* eslint-disable-next-line no-console */
    console.error('[Migration Cutover] Error:', error);
