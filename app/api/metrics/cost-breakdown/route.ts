...
function emptyResponse(startDate: string, endDate: string): CostBreakdownResponse {
  return {
...
export async function GET(req: NextRequest) {
  ...
  try {
    // Generate cache key
    const cacheKey = apiCacheKey('cost-breakdown', {
      start: startDate,
      end: endDate,
    ...
    const data = await cacheManager.getOrSet(
      cacheKey,
      () => fetchCostBreakdown(startDate, endDate, workspaceId, campaign, provider),
      CACHE_TTL.SHORT
    );
