/**
 * Campaign Group CSV Lead Import API
 *
 * POST /api/campaign-groups/[id]/import
 * Content-Type: multipart/form-data
 * Query: workspace_id
 *
 * Accepts a campaign group ID and imports leads with the group's
 * name as the campaign_name — so "Email 1", "Email 2", "Email 3"
 * are treated as sequences within the group, not separate campaigns.
 */
...
export async function POST(
  ...
  // ── Build insert payloads — use group name as campaign_name ──
  const campaignName: string = group.name;
  ...
  for (let i = 0; i < insertRows.length; i += BATCH_SIZE) {
    ...
    if (upsertError) {
      console.error('[CSV Group Import] Upsert error:', upsertError);
      return jsonResponse({
        success: false,
  ...
  return jsonResponse({
    ...
    total_rows_processed: rows.length,
    warnings: rowValidation.warnings,
    errors: [...parseErrors, ...rowValidation.errors].slice(0, 20),
  });
}
