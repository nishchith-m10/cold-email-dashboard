...
export function GenesisOnboardingWizard({
  ...
  // Floating bubble mode state — persists in localStorage
  const [mode, setMode] = useState<'bubble' | 'expanded'>(() => {
    if (typeof window !== 'undefined') {
      return (localStorage.getItem('onboarding-mode') as 'bubble' | 'expanded') || 'bubble';
    }
    return 'bubble';
  });
  ...
  const toggleMode = useCallback(() => {
    setMode(prev => {
      const next = prev === 'bubble' ? 'expanded' : 'bubble';
      localStorage.setItem('onboarding-mode', next);
      return next;
    });
  }, []);
  ...
  // --- Bubble mode: floating circular button ---
  if (mode === 'bubble') {
    return (
      <motion.button
        className="fixed bottom-6 right-6 z-50 h-14 w-14 rounded-full bg-accent-primary shadow-2xl flex items-center justify-center"
    ...
    <AnimatePresence>
      <motion.div
        className={cn(
          ...
          "md:left-auto md:bottom-6 md:right-6 md:w-[420px] md:max-h-[70vh] md:rounded-2xl"
        )}
      ...
      >
        ...
        <div className="flex items-center justify-between p-4 border-b border-border flex-shrink-0">
          ...
          <button
            onClick={toggleMode}
            className="p-1.5 hover:bg-surface-elevated rounded-lg transition-colors flex-shrink-0"
        ...
        <div className="h-1 bg-surface-elevated flex-shrink-0">
          <motion.div
          ...
          <div className="flex gap-1">
            {visibleStages.map((stage, index) => (
              <button
                key={stage.stage}
                onClick={() => {
                  const isAccessible = completedStages.has(stage.stage) || index === currentStageIndex || index === currentStageIndex + 1;
                  if (isAccessible) handleSkipToStage(index);
                }}
                className={cn(
                  'rounded-full transition-all',
                  index === currentStageIndex && 'w-4 h-1.5 bg-accent-primary',
                  index < currentStageIndex && completedStages.has(stage.stage) && 'w-1.5 h-1.5 bg-accent-success',
                  index > currentStageIndex && 'w-1.5 h-1.5 bg-border',
                  index < currentStageIndex && !completedStages.has(stage.stage) && 'w-1.5 h-1.5 bg-border'
                )}
              />
            ))}
          </div>
